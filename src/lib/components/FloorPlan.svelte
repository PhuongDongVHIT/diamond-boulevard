      ...
      {#if activeMainTab === 'floor'}
        <div class="flex justify-center gap-2 mb-6 flex-wrap" in:fade={{duration: 200}}>
          {#each floorPlans as plan}
            <button 
              class="px-6 py-2 rounded-lg font-semibold transition-colors {activeFloor === plan.id ? 'bg-bg-secondary text-accent border border-accent/20 shadow-sm' : 'text-text-secondary hover:text-text-color'}" 
        ...
        <div class="w-full bg-slate-50 dark:bg-slate-800/50 rounded-xl overflow-hidden min-h-[400px] flex items-center justify-center relative border border-border-color p-4">
          {#key activeFloor}
            <div class="w-full flex justify-center items-center" in:fade={{duration: 300, delay: 50}}>
              ...
              <div class="hidden flex-col items-center justify-center p-20 text-text-secondary border-2 border-dashed border-border-color rounded-xl w-full max-w-2xl">
                ...
                <p class="text-lg">Vui lòng tải ảnh <strong>{currentFloor.image.split('/').pop()}</strong> vào thư mục <code>static/images/</code></p>
              </div>
            </div>
          {/key}
        </div>
      ...
      {:else}
        <div class="flex justify-center gap-2 mb-6 flex-wrap" in:fade={{duration: 200}}>
          {#each apartments as apt}
            <button 
              class="px-6 py-2 rounded-lg font-semibold transition-colors {activeApartment === apt.id ? 'bg-bg-secondary text-accent border border-accent/20 shadow-sm' : 'text-text-secondary hover:text-text-color'}" 
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
          <div class="col-span-1 flex flex-col justify-center bg-bg-secondary p-8 rounded-xl border border-border-color shadow-[inset_0_2px_10px_rgba(0,0,0,0.02)]">
            <h3 class="text-2xl font-bold text-text-color mb-8 pb-4 border-b border-border-color">{currentApartment.name}</h3>
            
            <div class="space-y-8">
              <div>
                <div class="text-[0.85rem] text-text-secondary uppercase tracking-[1px] mb-2 font-semibold">Diện tích tim tường</div>
                <div class="text-4xl font-extrabold text-accent">{currentApartment.timtuong}</div>
              </div>
              
              <div>
                <div class="text-[0.85rem] text-text-secondary uppercase tracking-[1px] mb-2 font-semibold">Diện tích thông thủy</div>
                <div class="text-4xl font-extrabold text-text-color">{currentApartment.thongthuy}</div>
              </div>
            </div>
            
            <button class="mt-12 w-full py-4 rounded-xl font-bold bg-white dark:bg-slate-800 border border-border-color text-text-color hover:bg-accent hover:text-white hover:border-accent transition-colors shadow-sm">
              Nhận Báo Giá Chi Tiết
            </button>
          </div>
          
          <div class="col-span-1 lg:col-span-2 bg-slate-50 dark:bg-slate-800/50 rounded-xl overflow-hidden min-h-[400px] flex items-center justify-center relative border border-border-color p-6">
            {#key activeApartment}
              <div class="w-full flex justify-center items-center h-full" in:fade={{duration: 300, delay: 50}}>
                <!-- svelte-ignore a11y_img_redundant_alt -->
                <img src={currentApartment.image} alt={currentApartment.name} class="max-w-full max-h-[70vh] object-contain rounded-lg drop-shadow-sm" onerror={handleImageError} />
                <!-- Placeholder if image not found -->
                <div class="hidden flex-col items-center justify-center p-20 text-text-secondary border-2 border-dashed border-border-color rounded-xl w-full h-full min-h-[300px]">
                  <svg xmlns="http://www.w3.org/2000/svg" class="w-16 h-16 mb-4 opacity-50" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M3 10h18M3 14h18m-9-4v8m-7 0h14a2 2 0 002-2V8a2 2 0 00-2-2H5a2 2 0 00-2 2v8a2 2 0 002 2z" /></svg>
                  <p class="text-lg">Vui lòng tải ảnh <strong>{currentApartment.image.split('/').pop()}</strong> vào thư mục <code>static/images/</code></p>
                </div>
              </div>
            {/key}
          </div>
