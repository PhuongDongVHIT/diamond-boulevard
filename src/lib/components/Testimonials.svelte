<script>
  ...
  const testimonials = [
    {
      id: 1,
      quote: "Diamond Boulevard đã cung cấp cho chúng tôi một bất động sản thậm chí còn chưa kịp đưa ra thị trường. Sự kín đáo và mạng lưới của họ đơn giản là vô song.",
      author: "Jonathan R.",
    ...
    {
      id: 2,
      quote: "Đội ngũ dịch vụ khách hàng đã nâng tầm căn hộ penthouse của tôi và đạt được mức giá bán kỷ lục chỉ trong hai tuần kể từ khi niêm yết.",
      author: "Eleanor S.",
    ...
    {
      id: 3,
      quote: "Đỉnh cao của sự chuyên nghiệp. Họ thấu hiểu ngay lập tức các yêu cầu về lối sống và trải nghiệm cốt lõi của chúng tôi cho một dinh thự ven biển.",
      author: "Marcus W.",
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-12" use:staggerFadeUp={{ y: 40, stagger: 0.15 }}>
      {#each testimonials as item}
        <div class="bg-bg-color p-10 md:p-12 rounded-[1.5rem] shadow-[0_4px_30px_rgba(0,0,0,0.05)] relative border border-border-color transition-transform duration-300 hover:-translate-y-1 hover:shadow-[0_15px_30px_rgba(0,0,0,0.06)] gsap-reveal">
          ...
          <p class="text-[1.1rem] leading-relaxed text-text-color mb-10 italic">{item.quote}</p>
          <div class="flex items-center gap-5 border-t border-border-color pt-6">
            <div class="min-w-12 w-12 h-12 rounded-full bg-accent-light text-accent flex items-center justify-center font-bold text-[1.2rem]">
              {item.author.charAt(0)}
            </div>
            <div>
              <h4 class="m-0 text-[1.1rem] font-bold text-text-color">{item.author}</h4>
