...
<section
  class="relative min-h-[90vh] flex items-center bg-[url('/images/hero.png')] bg-cover bg-center bg-fixed"
>
  <div
    class="absolute inset-0 bg-gradient-to-r from-slate-900/90 to-slate-900/40"
  ></div>
  <div
    class="relative z-10 text-slate-50 w-full max-w-7xl mx-auto px-6 md:px-8"
  >
    <div class="max-w-[700px]">
      ...
      <form
        class="flex flex-col md:flex-row p-2 md:p-4 rounded-2xl md:rounded-[1rem] gap-2 md:gap-4 bg-white/10 backdrop-blur-md border border-white/20"
        onsubmit={handleSearch}
      >
        <div
          class="flex-1 flex items-center gap-4 px-4 py-2 border-b border-white/10 md:border-none mb-2 md:mb-0"
        >
          <svg
            class="text-slate-300 min-w-6 min-h-6"
