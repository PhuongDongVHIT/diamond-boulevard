...
<div class="relative w-screen h-screen overflow-hidden bg-bg-color gsap-reveal" use:fadeUp={{ y: 50, duration: 1 }}>
  {#each items as item, i}
    ...
    >
      ...
      <div class="relative z-20 text-center text-slate-50 max-w-[800px] px-8 transition-all duration-1000 ease-out pb-20 {i === currentIndex ? 'opacity-100 translate-y-0 scale-100 delay-[300ms]' : 'opacity-0 translate-y-5 scale-95'}">
        ...
        <button class="px-10 py-4 rounded-full font-semibold uppercase tracking-wider text-[1.1rem] transition-all bg-accent text-black bg-white/60 backdrop-blur-2xl  shadow-[0_4px_14px_rgba(239,101,34,0.3)] hover:bg-accent-hover hover:-translate-y-0.5 mt-4">Liên hệ tư vấn</button>
      </div>
    </div>
  {/each}
